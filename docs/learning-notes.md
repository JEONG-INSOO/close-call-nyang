...
가로용 `flex: 1`을 세로에 그대로 두면 남는 높이를 나누거나 줄어들 수 있다. 그래서 버튼 크기를 명시하고 기존 ScrollView와 위아래 패딩을 보존했다. 모든 버튼을 한 화면에 억지로 축소하는 대신 작은 화면에서 스크롤로 접근하게 했다. 랭킹등록완료! 문구/서버 확인 조건, 최고기록과 게임 규칙은 변경하지 않았다.

검증: typecheck, canonical8, 화면73개/7suites, 새 웹빌드 통과. 실제키입력으로 종료한 브라우저3viewport(1280x720/844x390/667x375)에서 모든 버튼 x/width 일치, 높이50, 간격>=9, 마지막버튼 접근 및 재도전 통과. 휴대폰 스크린샷도 확인했다. 공통규격9버튼 클릭과 busy재도전 차단은 단위검사로 보완했다. 운영 계정/점수 생성은 하지 않았다. Windows 테스트서버 정리 대기는 이번에 띄운3개PID만 확인·종료하여 해결했다.

학습 질문: 세로배치에서 flex1이 왜 위험할까? 크기공유와 onPress공유의 차이는? 스크롤 아래 버튼도 접근가능한지 어떻게 검사할까? 추가 제품 결함은 발견되지 않았다. 재배포 결과는 별도로 기록한다.

## 2026-09-25 · 랭킹 데이터는 유지하고 완료 안내만 줄이기

사용자 요청대로 ResultScreen에서 검증된 성공률·온라인 최고 기록·등수 상세 Text를 제거하고, ko.rankingSubmitted를 `랭킹등록완료!`로 바꿨다. 기본 프로젝트 성공률과 기기 최고 기록은 유지한다. 표시하지 않는 문자열 두 개도 참조가 없는지 확인한 뒤 삭제했다.

핵심 조건 `submissionState === 'submitted' && receipt`는 그대로다. 서버 제출 → 확인 응답(receipt) → 완료 문구라는 흐름을 보존해야 통신 대기나 실패를 등록 성공으로 오해하지 않는다. receipt의 score/bestScore/rank나 서버 API를 삭제한 것이 아니며, 별도 랭킹 화면도 그대로다.

타입검사·규칙사본8개 확인, 화면72개/7suites와 랭킹186개/12suites 통과. 순위3/null 모두 상세행 부재와 기본점수 보존을 검사하고, receipt가 없는 submitted/recording/local/unranked가 성공으로 표시되지 않는지 확인했다. 브라우저 셀렉터도 새 문구로 변경하고 mock API 전용 빌드를 재생성하여 휴대폰 가로화면 시나리오2개 통과·스크린샷 확인. 실제운영 쓰기/iPhone/공개사이트 재배포 검증은 아니다.

브라우저 시나리오는 둘 다 통과했지만 Windows에서 테스트 서버 정리가 대기했다. 실행 당시 출력된3개 서버의 PID/명령을 다시 확인하고 그 프로세스만 종료하여 runner exit0/2passed(1.8분)를 확인했다. PowerShell Stop-Process 오류는 Node process.kill로 우회했다. 다른 서버나 사용자 데이터는 건드리지 않았다. 테스트 assertion 통과와 실행기 정상 종료를 구분해야 한다.

이번에 새 제품 결함은 발견되지 않았다. 함께 갱신해야 한 부분은 옛 완료 문구를 찾던 E2E 셀렉터다. 학습 질문: receipt를 왜 유지할까? 화면 상세행 제거와 DB 데이터 삭제는 어떻게 다를까? 성공뿐 아니라 대기·실패 상태도 검사해야 하는 이유는 무엇일까?

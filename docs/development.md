...
P01-T01 개발 기반입니다. 제목과 준비 상태를 보여 주는 첫 화면만 있으며, 균형 물리·조작·랭킹은 아직 구현하지 않았습니다. 기존 스타터 README와 예시는 그대로 보존했습니다.
...
- 웹 산출물: `npm.cmd run web:export`. 현재 dist는 로컬 기반 확인용이며 GitHub Pages 하위 경로와 배포 workflow는 후속 작업입니다.
...
- 광고 가상 흐름은 후속 작업입니다. 현재 가드는 `isDev && enableMockAd === 'true'`이며 production은 공개 환경변수만 바꿔도 켜지지 않습니다.
...
- TypeScript6의 테스트 전역 타입을 명시했습니다. Jest는 네이티브 Worklets 경계와 safe area만 모의 처리합니다. 실제 폰의 UI 스레드 동작을 검증한 것으로 해석하지 않습니다.

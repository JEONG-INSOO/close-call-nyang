...
- **Execution Flow / Logic**:
  ...
  6. GET leaderboard cache30sec **per caller** or split public list+private me; do not cache one caller's isMe/me for another. Rate limits server-side; CORS allow deployed `https://mocca.github.io` (origin has no repo path), localhost dev only staging. Native requests without Origin are permitted: public GET leaderboard needs no JWT under anonymous read limits, whereas protected routes require verified JWT. CORS is not authentication.

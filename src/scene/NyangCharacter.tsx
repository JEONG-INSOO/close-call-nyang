...
const OUTLINE = 4.5;
const DETAIL = 2;
const PAW = 'M-15 8 Q-15 0 -6 0 H6 Q15 0 15 8 Q16 18 6 19 H-6 Q-16 18 -15 8Z';
...
const Outfit = memo(function Outfit({ id }: { id: RewardId }) {
  return (
...
function RewardCat({ frame, id }: Pick<SceneProps, 'frame'> & { id: RewardId }): React.JSX.Element {
  ...
  const leftPadProps = useAnimatedProps<GProps>(() => {
    const exposure = frame.value.fallen ? 1 : 0.62 + 0.38 * Math.max(0, strideFor('game', frame.value.distanceM));
  ...
  const tailProps = useAnimatedProps<GProps>(() => ({
    transform: svgTransform(-frame.value.angleRad * 14 + strideFor('game', frame.value.distanceM) * 4, -46, -47),
  }), [frame], svgTransformAdapter);
  ...
  return <>
    ...
    <AnimatedG testID="leg-left" animatedProps={leftLegProps}>
      <Path testID="paw-left" d={PAW} fill={NYANG_COLORS.fur} />
    ...
    <AnimatedG testID="leg-right" animatedProps={rightLegProps}>
      <Path testID="paw-right" d={PAW} fill={NYANG_COLORS.fur} />
    ...
    <G testID="plush-head">
      ...
      <AnimatedG animatedProps={faceProps}><Face id={id} /></AnimatedG>
